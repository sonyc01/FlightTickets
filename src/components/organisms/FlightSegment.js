...
import {IonCard,IonCardHeader,IonRow,IonCol,IonCardSubtitle,IonList,IonItem,IonLabel,IonAvatar,IonIcon, IonCardContent, IonGrid} from '@ionic/react';
    ...
    return(
        <IonList>
            {segments.map((item)=>{
                ...
                const departureTimeString=departureDate.toLocaleTimeString("sk-SK").slice(0,-3)
                const arrivalDate=new Date(item.arrival.at)
                const arrivalDateString=arrivalDate.toLocaleDateString("en-US")
                const arrivalTimeString=arrivalDate.toLocaleTimeString("sk-SK").slice(0,-3)
